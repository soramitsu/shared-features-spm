...
enum ExtrinsicCheck: String, CaseIterable {
    ...
    case checkMetadataHash = "frame_metadata_hash_extension::CheckMetadataHash"
    ...
    /// name
    private static var overridenTypes = ThreadSafeDictionary<UInt32, [String: String]>()
    ...
    static func from(string: String, runtimeMetadata: RuntimeMetadata) -> Self? {
        ...
        var overridenTypes = overridenTypes.value(for: runtimeMetadata.metaReserved) ?? [:]
        ...
        for check in Self.allCases {
               ...
               checkTypeName == typeName
            {
                ...
                self.overridenTypes.setValue(overridenTypes, for: runtimeMetadata.metaReserved)
