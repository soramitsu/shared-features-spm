...
public protocol DexManagerStorage {
    ...
    func subscribeDexInfos(chain: ChainModel) async
        -> AsyncThrowingStream<CachedStorageResponse<DexInfoByDexId>, Error>
}
...
public final class DexManagerStorageDefault: DexManagerStorage {
    ...
    public func dexInfos(chain: ChainModel) async throws -> DexInfoByDexId {
        ...
        let dexInfos: DexInfoByDexId? = try await storageRequestPerformer.performPrefix(
            baseAssetIdsRequest,
            chain: chain
        )
    ...
    public func subscribeDexInfos(chain: ChainModel) async
        -> AsyncThrowingStream<CachedStorageResponse<DexInfoByDexId>, Error>
    {
        let baseAssetIdsRequest = DexManagerDexInfosStorageRequest()
        let dexInfos: AsyncThrowingStream<CachedStorageResponse<DexInfoByDexId>, Error> =
