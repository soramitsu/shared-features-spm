...
final class XcmChainsConfigFetcherTests: XCTestCase {
    ...
    override func setUp() {
        ...
        chainRegistry.getChainsReturnValue = [TestData.firstChain, TestData.secondChain, TestData.errorChain]
    ...
    func testGetAvailableOriginalChains() async throws {
        // arrange
        let assetSymbol: String = "xc0"
        let destinationChainId: ChainModel.Id = "1"
        
        // act
        let chainIds = try await fetcher?.getAvailableOriginalChains(assetSymbol: assetSymbol,
    ...
    func testGetAvailableOriginalChainsWithError() async throws {
        ...
        // act
        do {
            try await fetcher?.getAvailableOriginalChains(assetSymbol: assetSymbol,
                                                          destinationChainId: destinationChainId)
        } catch {
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingRemoteChainsResult.localizedDescription)
        }
    }
    ...
    func testGetAvailableAssets() async throws {
        ...
        // act
        let assets = try await fetcher?.getAvailableAssets(originalChainId: originalChainID,
                                                           destinationChainId: destinationChainId)
    ...
    func testGetAvailableAssetsWithError() async throws {
        ...
        // act
        do {
            try await fetcher?.getAvailableAssets(originalChainId: originalChainID,
                                                  destinationChainId: destinationChainId)
        } catch {
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingRemoteChainsResult.localizedDescription)
    ...
    func testGetAvailableDestinationChains() async throws {
        // arrange
        let originalChainId: ChainModel.Id = "0"
        let assetSymbol: String = "xc1"
        
        // act
        let chainIds = try await fetcher?.getAvailableDestinationChains(originalChainId: originalChainId,
    ...
    func testGetAvailableDestinationChainsWithError() async throws {
        ...
        // act
        do {
            try await fetcher?.getAvailableDestinationChains(originalChainId: originalChainId,
                                                             assetSymbol: assetSymbol)
        } catch {
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingRemoteChainsResult.localizedDescription)
    ...
    func testGetVersionWithError() async throws {
        ...
        do {
            ...
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingRemoteXcmVersion.localizedDescription)
        }
    }
...
extension XcmChainsConfigFetcherTests {
    enum TestData {
        static let firstChain = ChainModel(rank: 0,
                                           disabled: false,
                                           chainId: "0",
                                           paraId: "1001",
                                           name: "test1",
        ...
        static let secondChain = ChainModel(rank: 1,
                                            disabled: false,
                                            chainId: "1",
                                            paraId: "1002",
                                            name: "test2",
                                            xcm: XcmChain(xcmVersion: .V1,
                                                          destWeightIsPrimitive: true,
                                                          availableAssets: [.init(id: "1", symbol: "1")],
        ...
        static let errorChain = ChainModel(rank: 2,
                                           disabled: false,
                                           chainId: "2",
                                           paraId: "1",
                                           name: "test3",
