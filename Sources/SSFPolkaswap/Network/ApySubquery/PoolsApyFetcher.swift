...
public protocol PoolsApyFetcher {
    ...
    func subscribe(poolIds: [String]) async throws -> AsyncThrowingStream<[CachedNetworkResponse<PoolApyInfo>], Swift.Error>
}
...
final class PoolsApyFetcherDefault: PoolsApyFetcher {
    ...
    private func subscribeNewApyInfos(
        ...
        return AsyncThrowingStream<CachedNetworkResponse<SoraSubqueryApyInfoPage>, Swift.Error> { continuation in
            Task {
                guard let url else {
                ...
                for try await poolsApy in stream {
                    ...
                    switch value {
                    case let .data(apys):
                        let response = CachedNetworkResponse(value: apys.entities, type: poolsApy.type)
