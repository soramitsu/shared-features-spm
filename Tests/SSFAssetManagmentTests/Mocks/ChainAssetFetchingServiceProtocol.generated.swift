...
actor ChainAssetFetchingServiceProtocolMock: ChainAssetFetchingServiceProtocol {
    ...
    var fetchFiltersSortsForceUpdateCalled: Bool {
        fetchFiltersSortsForceUpdateCallsCount > 0
    }

    var fetchFiltersSortsForceUpdateReceivedArguments: (
        filters: [AssetFilter],
        sorts: [AssetSort],
        forceUpdate: Bool
    )?
    var fetchFiltersSortsForceUpdateReceivedInvocations: [(
        filters: [AssetFilter],
        sorts: [AssetSort],
        forceUpdate: Bool
    )] = []
    ...
    func fetch(filters: [AssetFilter], sorts: [AssetSort], forceUpdate: Bool) -> [ChainAsset] {
        ...
        fetchFiltersSortsForceUpdateReceivedArguments = (
            filters: filters,
            sorts: sorts,
            forceUpdate: forceUpdate
        )
        fetchFiltersSortsForceUpdateReceivedInvocations.append((
            filters: filters,
            sorts: sorts,
            forceUpdate: forceUpdate
        ))
        return fetchFiltersSortsForceUpdateClosure
            .map { $0(filters, sorts, forceUpdate) } ?? fetchFiltersSortsForceUpdateReturnValue
    }
}
