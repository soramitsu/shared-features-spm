...
extension MnemonicExportService: MnemonicExportServiceProtocol {
    func fetchExportDataFor(wallet: MetaAccountModel, accounts: [ChainAccountInfo]) -> [MnemonicExportData] {
        ...
        for chainAccount in accounts {
            ...
            let cryptoType = chainAccount.account.isEthereumBased ? nil : chainAccount.account.cryptoType
    ...
    func fetchExportDataFor(address: String, chain: ChainModel, wallet: MetaAccountModel) throws -> MnemonicExportData {
        ...
        let accountId = chainRespone.isChainAccount ? wallet.fetch(for: chain.accountRequest())?.accountId : nil
