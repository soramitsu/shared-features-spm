...
class GIDSignInMock: GIDSignIn {
    ...
    var signInReceivedArguments: (withPresenting: UIViewController, hint: String?, additionalScopes: [String]?, completion: ((GIDSignInResult?, Error?) -> Void)?)?
    var signInClosure: ((UIViewController, String?, [String]?, ((GIDSignInResult?, Error?) -> Void)?) -> Void)?
    ...
    override func signIn(withPresenting presentingViewController: UIViewController,
        ...
        signInCallsCount += 1
        signInReceivedArguments = (withPresenting: presentingViewController, hint: hint, additionalScopes: additionalScopes, completion: completion)
