...
final class AccountInfoFetching: AccountInfoFetchingProtocol {
    ...
    func fetch(
        ...
        let executeOperation = ClosureOperation { [weak self] in
            let accountInfoOperations = try createAccountInfoOperationsOperation
                .extractNoCancellableResultData()
            let accountInfoDependencies = accountInfoOperations.compactMap { $0.dependencies }
                .reduce(
                    [],
                    +
                )
            let accountInfoSubdependencies = accountInfoDependencies.compactMap { $0.dependencies }
                .reduce(
                    [],
                    +
                )
        ...
        createAccountInfoOperationsOperation.dependencies
            .forEach { executeOperation.addDependency($0) }

        operationQueue.addOperations(
            dependencies + [createAccountInfoOperationsOperation, executeOperation],
            waitUntilFinished: false
        )
    }
...
private extension AccountInfoFetching {
    private func prepareAccountInfoOperationsOperation(
        ...
        let positiveBalanceOperations =
            ClosureOperation<[ClosureOperation<[ChainAsset: AccountInfo?]>]> {
                let rawResult = try fetchOperation.extractNoCancellableResultData()
                let chainAssetsByKeys = try mapChainAssetsWithKeysOperation
                    .extractNoCancellableResultData()

                let operations: [ClosureOperation<[ChainAsset: AccountInfo?]>] = rawResult
                    .compactMap { [weak self] accountInfoStorageWrapper in
    ...
    func createDecodingOperation<T: Decodable>(
        ...
        guard let runtimeCodingService = chainRegistry.getRuntimeProvider(
