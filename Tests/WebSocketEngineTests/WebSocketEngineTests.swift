...
final class WebSocketEngineTests: XCTestCase {
    ...
    func testConnectedOnTheSecondAttempt() throws {
        ...
        webSocketEngine.didReceive(
            event: .disconnected("test", 0),
            client: strategy.currentConnection
        )
        webSocketEngine.didReceive(event: .connected([:]), client: strategy.currentConnection)
    ...
    func testConnectedOnTheSecondLoop() throws {
        ...
        webSocketEngine.didReceive(
            event: .disconnected("disconnect from first url", 0),
            client: strategy.currentConnection
        )
        webSocketEngine.didReceive(
            event: .disconnected("disconnect from second url", 1),
            client: strategy.currentConnection
        )
    ...
    func testConnectedOnTheThirdLoop() throws {
        ...
        webSocketEngine.didReceive(
            event: .disconnected("disconnect 1", 0),
            client: strategy.currentConnection
        )
        webSocketEngine.didReceive(
            event: .disconnected("disconnect 2", 1),
            client: strategy.currentConnection
        )
        strategy.didTrigger(scheduler: SchedulerMock())

        webSocketEngine.didReceive(
            event: .disconnected("disconnect 3", 2),
            client: strategy.currentConnection
        )
        webSocketEngine.didReceive(
            event: .disconnected("disconnect 4", 3),
            client: strategy.currentConnection
        )
        strategy.didTrigger(scheduler: SchedulerMock())
    ...
    func testResendAfterReconnection() throws {
        ...
        webSocketEngine.didReceive(
            event: .disconnected("disconnected", 0),
            client: strategy.currentConnection
        )
        XCTAssertEqual(webSocketEngine.pendingRequests.count, 1)
        ...
        webSocketEngine.didReceive(
            event: .disconnected("desconnected", 1),
            client: strategy.currentConnection
        )
        XCTAssertEqual(webSocketEngine.pendingRequests.count, 1)
    ...
    func testErrorHandling() throws {
        ...
        webSocketEngine.didReceive(
            event: .error(WebSocketEngineTestsError.trigger),
            client: strategy.currentConnection
        )
        XCTAssertEqual(webSocketEngine.inProgressRequests.count, 0)
    ...
    func testWaitingHandling() throws {
        ...
        webSocketEngine.didReceive(event: .connected([:]), client: strategy.currentConnection)
        webSocketEngine.didReceive(
            event: .waiting(error: WebSocketEngineTestsError.trigger),
            client: strategy.currentConnection
        )
    ...
    func testReconnectingSuggestedFalse() throws {
        ...
        webSocketEngine.didReceive(event: .connected([:]), client: strategy.currentConnection)
        webSocketEngine.didReceive(
            event: .reconnectSuggested(false),
            client: strategy.currentConnection
        )
...
private struct ReconnectionStrategyProtocolMock: ReconnectionStrategyProtocol {
    public func reconnectAfter(attempt _: Int) -> TimeInterval? {
        nil
    }
}

private class SchedulerMock: SchedulerProtocol {
