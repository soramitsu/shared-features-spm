...
final class XcmDestinationTests: XCTestCase {
    ...
    func testDetermineChainTypeWithError() throws {
        // act
        XCTAssertThrowsError(try XcmChainType.determineChainType(for: TestData.errorChain)) { error in
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.invalidParachainRange.localizedDescription)
        }
    }
}
