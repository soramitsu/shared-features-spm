...
public class BackupFileFactory: NSObject, BackupFileFactoryProtocol {
    ...
    public func createFile(from account: OpenBackupAccount, password: String) throws -> URL {
        ...
        let encodedPassphrase = try service.createEncryptedData(with: password, message: account.passphrase)
        let encodedSubstrateDerivationPath = try service.createEncryptedData(with: password, message: account.substrateDerivationPath)
        let encodedEthDerivationPath = try service.createEncryptedData(with: password, message: account.ethDerivationPath)
        let encodedSubstrateSeed = try service.createEncryptedData(with: password, message: account.encryptedSeed?.substrateSeed)
        let encodedEthSeed = try service.createEncryptedData(with: password, message: account.encryptedSeed?.ethSeed)
