...
enum XcmJunction: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        guard let onlyKey = allKeys.popFirst(), allKeys.isEmpty else {
            throw DecodingError.typeMismatch(XcmJunction.self, DecodingError.Context.init(codingPath: container.codingPath, debugDescription: "Invalid number of keys found, expected one.", underlyingError: nil))
        }
        switch onlyKey {
        case .parachain:
            self = XcmJunction.parachain(try container.decode(UInt32.self, forKey: .parachain))
        case .accountId32:
        ...
        case .accountIndex64:
            let accountIndex64Value = try container.decode(AccountIndexValue.self, forKey: .accountIndex64)
        ...
        case .palletInstance:
            self = XcmJunction.palletInstance(try container.decode(UInt8.self, forKey: .palletInstance))
        case .generalIndex:
            let index = try container.decode(Int.self, forKey: .generalIndex)
            guard let generalIndex = BigUInt("\(index)") else {
                let context = DecodingError.Context.init(codingPath: container.codingPath, debugDescription: "general index not found", underlyingError: nil)
...
extension XcmJunction: Equatable {
    static func == (lhs: XcmJunction, rhs: XcmJunction) -> Bool {
        switch (lhs, rhs) {
        case (let .parachain(lhsValue), let .parachain(rhsValue)):
            return lhsValue == rhsValue
        case (let .accountId32(lhsValue), let .accountId32(rhsValue)):
            return lhsValue == rhsValue
        case (let .accountIndex64(lhsValue), let .accountIndex64(rhsValue)):
            return lhsValue == rhsValue
        case (let .accountKey20(lhsValue), let .accountKey20(rhsValue)):
            return lhsValue == rhsValue
        case (let .palletInstance(lhsValue), let .palletInstance(rhsValue)):
            return lhsValue == rhsValue
        case (let .generalIndex(lhsValue), let .generalIndex(rhsValue)):
            return lhsValue == rhsValue
        case (let .generalKey(lhsValue), let .generalKey(rhsValue)):
            return lhsValue == rhsValue
        case (let .generalKeyV3(lhsValue), let .generalKeyV3(rhsValue)):
            return lhsValue == rhsValue
