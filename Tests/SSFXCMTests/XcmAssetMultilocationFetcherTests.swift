...
final class XcmAssetMultilocationFetcherTests: XCTestCase {
    ...
    func testVersionedMultilocation() async throws {
        ...
        // act
        let assetMultiplication = try await fetcher?.versionedMultilocation(originAssetId: "0", destChainId: "3")
    ...
    func testVersionedMultilocationWithAssetError() async throws {
        ...
        // act
        do {
            let assetMultiplication = try await fetcher?.versionedMultilocation(originAssetId: "1", destChainId: "2")
        } catch {
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingAssetLocationsResult.localizedDescription)
        }
    }
    ...
    func testVersionedMultilocationWithRetryError() async throws {
        ...
        do {
            ...
            // assert
            XCTAssertEqual(error.localizedDescription, XcmError.missingLocalAssetLocations.localizedDescription)
...
extension XcmAssetMultilocationFetcherTests {
    ...
    private func createInitOperation() -> BaseOperation<[RemoteAssetMultilocation]> {
        let asset = AssetMultilocation(id: "1",
                                       symbol: "2",
                                       interiors: [.onlyChild])
        
        let multilocation = RemoteAssetMultilocation(name: "3",
                                                     chainId: "4",
                                                     assets: [asset])
    ...
    private func createVersionedOperation() -> BaseOperation<[RemoteAssetMultilocation]> {
        let multilocation = RemoteAssetMultilocation(name: "2",
                                                     chainId: "3",
